		...
		config = function()
			...
			local on_attach = function(client, bufnr)
				...
				map(
					...
					vim.tbl_extend("force", opts, { desc = "LSP: Idź do definicji" })
				)
				map(
					"n",
					"gr",
					vim.lsp.buf.references,
					vim.tbl_extend("force", opts, { desc = "LSP: Pokaż referencje" })
				)
				...
				map(
					...
					"<leader>rn",
					vim.lsp.buf.rename,
					vim.tbl_extend("force", opts, { desc = "LSP: Zmień nazwę" })
				)
			end
				...
				handlers = {
					...
					["jdtls"] = function()
						require("lspconfig").jdtls.setup({
							cmd = {
								"jdtls",
								"--jvm-arg=-javaagent:/home/kacper/.local/share/nvim/mason/packages/jdtls/lombok.jar",
							},
